...
void st_remove(struct st_node **root, struct st_node *del)
{
    if (st_right(del)) {
        ...
        st_replace_right(del, least); // AAAA; 替換 least 到 del
        st_update(root, st_right(least)); // BBBB; 從 least->right 開始更新
        return;
    }
    ...
    if (st_left(del)) {
        ...
        st_replace_left(del, most); // CCCC; 替換 most 到 del
        st_update(root, st_left(most)); // DDDD; 從 most->left 開始更新子樹
        return;
    }
    ...
    st_update(root, parent); // EEEE; 更新 parent
}
...
static void __treeint_dump(struct st_node *n, int depth)
{
    ...
    __treeint_dump(st_left(n), depth + 1); // __treeint_dump(FFFF, depth + 1);

    struct treeint *v = treeint_entry(n);
    printf("%d\n", v->value);

    __treeint_dump(st_right(n), depth + 1); // __treeint_dump(GGGG, depth + 1);
}
